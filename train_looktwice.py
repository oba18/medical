...
class Trainer(object):
    def __init__(self, batch_size=32, optimizer_name="Adam", lr=1e-3, weight_decay=1e-5,
        ...
        ## ***Define Your Model***
        self.model = ModelingLookTwice(self.num_classes)
    ...
    def _run_epoch(self, epoch, mode="train", leave_progress=True, use_optuna=False):
        ...
        for i, sample in enumerate(data_loader):
            ...
            if mode=="train":
                ...
                loss2 = self.criterion(output2, target)
                loss = loss1 + loss2
                loss.backward()
                self.optimizer.step()
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default="looktwice", metavar='Name', help='model name (default: model01)')
